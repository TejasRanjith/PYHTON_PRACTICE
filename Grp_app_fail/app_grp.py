...
def popup(main,str1):
    tk.Label(main,text = str1)


def chck_dict(e,p,window):
    ...
    for k in main_datadict:
        if k == email_get:
            if main_datadict[k][1] == pass_get:
                tk.Label(window,text = str(main_datadict[k])).grid(row = 2,column = 2)
                # tk.Label()
                # messagebox.showinfo("INFO",str(main_datadict[k]))
                # messagebox.showinfo()
                # print(str(main_datadict[k]))
...
def e_get():
    ...
    if act_get.lower() == "login":
        ...
        func = lambda: chck_dict(e_email,e_pass,login)
        
        b_login = tk.Button(login,text = "Login",command = func)    # herererererererererere
